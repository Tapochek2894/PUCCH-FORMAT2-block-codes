# PUCCH F2 Codec

Реализация кодера для **5G PUCCH Format 2** (Physical Uplink Control Channel). Включает кодирование, модуляцию QPSK, передачу через канал AWGN, демодуляцию и декодирование.

---

## 📋 Требования

| Компонент | Версия |
| ----------- | -------- |
| Компилятор | g++ ≥ 11.4 или clang++ ≥ 14 |
| Стандарт C++ | C++17 |
| Библиотеки | STL, GTest, nlohmann/json |

---

## 🚀 Быстрый старт

```bash
# Сборка
make

# Запуск тестов
make run

# Запуск с входным файлом
make run input.json

# Очистка
make clean
```

---

## 📁 Структура проекта

``` bash
pucch_f2_codec/
├── include/              # Заголовочные файлы
│   ├── encoder.hpp
│   ├── decoder.hpp
│   ├── modulator.hpp
│   ├── demodulator.hpp
│   └── channel.hpp
├── src/                  # Исходный код
│   ├── main.cpp
│   ├── encoder.cpp
│   ├── decoder.cpp
│   ├── modulator.cpp
│   ├── demodulator.cpp
│   └── channel.cpp
├── build/                # Артефакты сборки (игнорируется в Git)
├── tests/                # Юнит-тесты (GTest)
├── bench/                # Бенчмарки
├── Makefile
├── input.json            # Пример входных данных
└── README.md
```

---

## 🔧 Режимы работы

### 1. Кодирование

Кодирование информационных битов в QPSK-символы.

**Вход:**

```json
{
    "mode": "coding",
    "num_of_pucch_f2_bits": 11,
    "pucch_f2_bits": [1, 0, 1, 1, 0, 0, 1, 0, 1, 1, 0]
}
```

**Выход:**

```json
{
    "mode": "coding",
    "num_of_pucch_f2_bits": 11,
    "qpsk_symbols": ["0.707+0.707j", "-0.707+0.707j", ...]
}
```

---

### 2. Декодирование

Декодирование принятых QPSK-символов в информационные биты.

**Вход:**

```json
{
    "mode": "decoding",
    "num_of_pucch_f2_bits": 11,
    "qpsk_symbols": ["0.707+0.707j", "-0.707+0.707j", ...]
}
```

**Выход:**

```json
{
    "mode": "decoding",
    "num_of_pucch_f2_bits": 11,
    "pucch_f2_bits": [1, 0, 1, 1, 0, 0, 1, 0, 1, 1, 0]
}
```

---

### 3. Симуляция работы системы

Симуляция Монте-Карло для заданного SNR для вычисления BLER.

**Вход:**

```json
{
    "mode": "channel simulation",
    "num_of_pucch_f2_bits": 11,
    "snr_db": 5.0,
    "iterations": 1000
}
```

**Выход:**

```json
{
    "mode": "channel simulation",
    "num_of_pucch_f2_bits": 11,
    "snr_db": 5.0,
    "iterations": 1000,
    "bler": 0.1500,
    "success": 850,
    "failed": 150
}
```

---

## 🛠 Сборка

| Команда | Описание |
| --------- | ---------- |
| `make` | Сборка release-версии |
| `make clean` | Очистка артефактов сборки |
| `make run [args]` | Запуск программы с аргументами |
| `make test` | Запуск юнит-тестов (требуется GTest) |
| `make bench` | Запуск бенчмарков |
| `make help` | Показать справку |

---

## 📐 Технические детали

### Кодирование

- **Алгоритм:** код Рида-Маллера (20, n) из 3GPP TS 38.212
- **Длины кода:** 2, 4, 6, 8, 11 бит
- **Кодовое слово:** всегда 20 бит
- **Порождающая матрица:** A₂₀ₓ₁₃, усечение слева для n < 13

### Модуляция

- **Тип:** QPSK с кодом Грея
- **Нормализация:** Единичная мощность символа (Es = 1)
- **Маппинг:** `0 → +1/√2`, `1 → -1/√2` на каждой оси

### Канал

- **Тип:** С аддитивным белым гаусовским шумом
- **Шум:** Гауссовский, независимый по синфазной и квадратурной составляющим
- **Формула шума:** `σ = √(1 / (2 × SNR_linear))`

### Демодуляция

- **Тип:** Мягкая
- **Выход:** Логарифмическое отношение правдоподобия (LLR) для каждого бита
- **Формула:** `LLR = -2√2 × SNR_linear × coordinate`

### Декодирование

- **Алгоритм:** Максимум правдоподобия (полный перебор)
- **Оптимизация:** Предрасчитанная таблица кодовых слов
- **Сложность:** O(2ⁿ) операций на декодирование

---

## 🧪 Тестирование

### Запуск тестов

```bash
make test
```

### Примеры тестов

- Кодирование/декодирование без шума (BLER = 0)
- Проверка всех длин кода {2, 4, 6, 8, 11}
- Monte-Carlo симуляция BLER vs SNR

---

## 📊 Бенчмарки

### Запуск

```bash
make bench
```

### Производительность декодера (TODO)

| Длина кода (n) | Кодовых слов | Операций на Decode() | Время (нс) |
| ---------------- | -------------- | ---------------------- | ------------ |

---

## 📚 Ссылки

- **3GPP TS 38.212** — Multiplexing and channel coding (Section 6.3.2.4)
- **3GPP TS 38.211** — Physical channels and modulation (Section 6.3.2)
- **nlohmann/json** — <https://github.com/nlohmann/json>
